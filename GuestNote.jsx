...
function DietaryTag({ text, type }) {
  const styles = {
    vegan: { bg: "#C8E6C9", color: "#2E7D32", label: "🌱" },
    vegetarian: { bg: "#81C784", color: "#fff", label: "🥬" },
    pescatarian: { bg: "#81D4FA", color: "#01579B", label: "🐟" },
  };
...
export default function GuestNoteApp() {
  ...
  useEffect(() => {
    (async () => {
      try {
        let g, e, a, ac;
        try { g = JSON.parse((await window.storage.get("gn-guests", false)).value); } catch { g = null; }
        try { e = JSON.parse((await window.storage.get("gn-events", false)).value); } catch { e = null; }
        try { a = JSON.parse((await window.storage.get("gn-audit", false)).value); } catch { a = null; }
        try { ac = JSON.parse((await window.storage.get("gn-accounts", false)).value); } catch { ac = null; }
        ...
        if (!g || !g.length) {
          ...
          await window.storage.set("gn-guests", JSON.stringify(g), false);
          await window.storage.set("gn-events", JSON.stringify(e), false);
          await window.storage.set("gn-audit", JSON.stringify(a), false);
        }
        if (!ac || !ac.length) {
          ac = [{ id: uid(), name: "Admin", username: "admin", password: "admin123", role: "Admin" }];
          await window.storage.set("gn-accounts", JSON.stringify(ac), false);
        }
        setGuests(g || []);
        setEvents(e || []);
        setAuditLog(a || []);
        ...
        setAccounts([{ id: uid(), name: "Admin", username: "admin", password: "admin123", role: "Admin" }]);
      } finally {
        setLoading(false);
      }
    })();
...
function AccountsScreen({ accounts, currentStaffId, onBack, onAdd, onUpdate, onDelete }) {
  ...
  const save = () => {
    ...
    if (editing === "new") {
      ...
      onUpdate({ ...existing, name: trimmedName, username: trimmedUser, role, password: password ? password : existing.password });
    }
    setEditing(null);
  };
          ...
          ${attendees.map((x) => {
            ...
            return `
              <div class="guest ${hasAllergy ? 'allergy' : ''}">
                ...
                <div class="name">${g.title} ${g.name}${g.vip ? ' ⭐' : ''}</div>
                ${g.allergies.map((a) => `<div class="allergy">⚠️ ${a.allergen.toUpperCase()} (${a.severity})</div>`).join('')}
                ${g.dietary.map((d) => `<div class="dietary">🥗 ${d.text}</div>`).join('')}
                ${g.serviceNotes.map((s) => `<div class="note">→ ${s.text}</div>`).join('')}
                ${g.preferences.map((p) => `<div class="note">❤️ ${p.text}</div>`).join('')}
                ${g.drinks.map((d) => `<div class="note">🍷 ${d.text}</div>`).join('')}
              </div>
            `;
          }).join('')}
          <div class="footer">
        ...
        {attendees.map((x) => {
          ...
          return (
            <div key={x.guestId} style={{
              ...
              {g.dietary.map((d) => (
                <div key={d.id} style={{ fontSize: 13.5, color: COLORS.green, marginBottom: 3 }}>🥗 {d.text}</div>
              ))}
              {g.serviceNotes.map((s) => (
                <div key={s.id} style={{ fontSize: 13.5, marginBottom: 3 }}>→ {s.text}</div>
              ))}
              {g.tablePreferences.map((s) => (
                <div key={s.id} style={{ fontSize: 13.5, marginBottom: 3 }}>🪑 {s.text}</div>
              ))}
              {g.preferences.map((p) => (
                <div key={p.id} style={{ fontSize: 13.5, marginBottom: 3 }}>❤️ {p.text}</div>
              ))}
              {g.drinks.map((d) => (
                <div key={d.id} style={{ fontSize: 13.5, marginBottom: 3 }}>🍷 {d.text}</div>
              ))}
            </div>
          );
        })}
            ...
            <div style={{ maxHeight: 200, overflowY: "auto", display: "flex", flexDirection: "column", gap: 6, borderRadius: 12, border: `1px solid ${COLORS.border}`, padding: 10 }}>
              {preview.map((g) => (
                <div key={g.id} style={{ fontSize: 12.5, padding: "6px 8px", background: COLORS.surfaceAlt, borderRadius: 8 }}>
                  ...
                  {g.allergies.length > 0 && <div style={{ color: COLORS.red }}>⚠️ {g.allergies.map((a) => a.allergen).join(", ")}</div>}
                </div>
              ))}
            </div>
          </div>
